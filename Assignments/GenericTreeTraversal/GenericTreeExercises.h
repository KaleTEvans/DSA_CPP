...
static void treeFactory(GenericTree<int>& tree) {
  ...
  tree.clear();
  tree.createRoot(4);
  // This is the full way to create a new pointer to TreeNode, since TreeNode is dependent
  // on a specific instance of the GenericTree<T> class template
  // it is much easier to just use auto in this case
  // typename GenericTree<int>::TreeNode* rt = tree.getRootPtr();
  auto rt = tree.getRootPtr();
  auto rightOfRoot = rt->addChild(8);
  rt->addChild(15);

  auto rightOfPrev = rightOfRoot->addChild(16);
  rightOfRoot->addChild(23);

  rightOfPrev->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while (!q.empty()) {
    ...
    for (auto topNode : temp->childrenPtrs) {
      q.push(topNode);
    }
    q.pop();
  }
